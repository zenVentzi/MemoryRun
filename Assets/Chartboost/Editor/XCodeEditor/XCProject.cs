...
//					result.AddEnergy( current.Value );
//			}
//			return result;
//		}
...
//			}
//			Debug.Log( "AddEnergy file result path: " + filePath );
...
//				PBXDictionary<PBXBuildPhase> currentPhase = GetBuildPhase( fileReference.buildPhase );
				...
				switch( fileReference.buildPhase ) {
					case "PBXFrameworksBuildPhase":
						...
						else if (!string.IsNullOrEmpty( absPath ) && Directory.Exists(absPath) && absPath.EndsWith(".framework") && tree.CompareTo("GROUP") == 0) { // Annt: AddEnergy framework search path for FacebookSDK
